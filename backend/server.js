...
const config = {
  user: process.env.DB_USER || "db_ac6cf3_gstscheme_admin",
  password: process.env.DB_PASSWORD || "Gst@2026",
  server: process.env.DB_SERVER || "sql5111.site4now.net",
  database: process.env.DB_NAME || "db_ac6cf3_gstscheme",
...
app.post("/sendotp", async (req, res) => {
  ...
  try {await sql.query `INSERT INTO OTPLOGIN (MOBILENO, OTP) VALUES (${mobile}, ${otp})`;
    const message = encodeURIComponent(`Dear Customer, Your Admin Login OTP is ${otp}. Please do not share this OTP. Regards, GS Thanga Maligai, Contact : 8300455155, 7598439019`);
